...
const sUSDSynthId = 0;
describe.skip('ModifyCollateral Withdraw - part 1', () => {
...
describe.skip('ModifyCollateral Withdraw - part 2: withdraw with open positions', () => {
  ...
  before('open positions', async () => {
    ...
    for (let i = 0; i < perpsMarkets().length; i++) {
      const perpsMarket = perpsMarkets()[i];
  ...
  it('assert something to make sure we have the right setup', () => {
    assertBn.equal(1, 1);
  });
