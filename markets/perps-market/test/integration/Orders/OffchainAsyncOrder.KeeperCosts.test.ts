...
describe('Settle Offchain Async Order test - Keeper gas cost', () => {
  ...
  before('set keeper costs', async () => {
    await keeperCostOracleNode().connect(owner()).setCosts(1111, 3333, 5555, 7777);
  });
  ...
  it('keeper costs are configured correctly', async () => {
    ...
    assertBn.equal(await keeperCostOracleNode().settlementCost(), 1111);
    assertBn.equal(await keeperCostOracleNode().flagCost(), 3333);
    assertBn.equal(await keeperCostOracleNode().liquidateCost(), 5555);
    assertBn.equal(await keeperCostOracleNode().elegibilityCost(), 7777);
  });

  describe('settelemt case', () => {
