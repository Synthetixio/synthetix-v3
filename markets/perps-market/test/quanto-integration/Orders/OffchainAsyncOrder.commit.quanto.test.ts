...
import { DepositCollateralData, depositCollateral, settleOrder } from '../../integration/helpers';
...
describe('Quanto Commit Offchain Async Order test', () => {
      ...
      perpsMarkets: [
        {
          ...
          token: 'snxETH',
          price: bn(1000),
          fundingParams: { skewScale: bn(100_000).div(10_000), maxFundingVelocity: bn(0) },
          quanto: {
            name: 'Bitcoin',
            token: 'snxBTC',
            price: bn(10_000),
  ...
  // for (let idx = 0; idx < 1; idx++) {
    ...
    describe(`Using ${testCase.name} as collateral`, () => {
      ...
      it('emit event', async () => {
        await assertEvent(
          ...
          `OrderCommitted(${ethMarketId}, 2, ${DEFAULT_SETTLEMENT_STRATEGY.strategyType}, ${bn(
            1
          ).div(10_000)}, ${bn(1050)}, ${startTime}, ${startTime + 2}, ${startTime + 5}, ${
            startTime + 5 + 120
      ...
      it('identifies the pending order', async () => {
        ...
        assertBn.equal(order.request.sizeDelta, bn(1).div(10_000));
        ...
        describe('can commit another order after expiration', () => {
          ...
          it('emit the order commited event', async () => {
            await assertEvent(
              ...
              `OrderCommitted(${ethMarketId}, 2, ${DEFAULT_SETTLEMENT_STRATEGY.strategyType}, ${bn(
                1
              ).div(10_000)}, ${bn(1050)}, ${secondOrderStartTime}, ${secondOrderStartTime + 2}, ${
          ...
          it('emit the order expired event', async () => {
            await assertEvent(
              tx,
              `PreviousOrderExpired(${ethMarketId}, 2, ${bn(1).div(10_000)}, ${bn(1050)}, ${startTime}, "${
                ethers.constants.HashZero
              }")`,
              systems().PerpsMarket
            );
