...
describe('Quanto Offchain Async Order test - fees', () => {
  const orderFees = {
  ...
  for (const testCase of testCases) {
    describe(`Using ${testCase.name} as collateral`, () => {
      ...
      describe('multiple orders ', () => {
        ...
        const initialLongSize = bn(2).div(10_000);
        ...
        describe('reduce position size (maker)', () => {
          ...
          const sizeDelta = bn(-1).div(10_000); // original size is 2, reduce by 1 => still long, but smaller
        ...
        describe('flip the order side (maker + taker)', () => {
          let feesPaidOnShort: Fees;
          const sizeDelta = bn(-3).div(10_000); // original size is 2, reduce by 3 => flip to short -1
