...
library AsyncOrder {
    ...
    function calculateFillPrice(
        ...
        int pdBefore = calculatePD(skew, skewScale);
        int pdAfter = calculatePD(skew + size, skewScale);

        int priceBefore = calculateAdjustedPrice(price, pdBefore);
        int priceAfter = calculateAdjustedPrice(price, pdAfter);

        return calculateAveragePrice(priceBefore, priceAfter);
    }

    // Calculates PD
    function calculatePD(int skew, uint skewScale) internal pure returns (int) {
        return skew.divDecimal(skewScale.toInt());
    }

    // Calculates the price with pd applied
    function calculateAdjustedPrice(uint price, int pd) internal pure returns (int) {
        return price.toInt() + (price.toInt().mulDecimal(pd));
    }

    // Calculates the average of two prices
    function calculateAveragePrice(int priceBefore, int priceAfter) internal pure returns (uint) {
