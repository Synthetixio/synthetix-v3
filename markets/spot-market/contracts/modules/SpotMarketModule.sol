...
contract SpotMarketModule is ISpotMarketModule, InitializableMixin {
    ...
    function buy(uint128 marketId, uint amountUsd) external override onlyIfInitialized returns (uint) {
    ...
    function sell(uint128 marketId, uint sellAmount) external override onlyIfInitialized returns (uint) {
        SpotMarketFactory.Data storage store = SpotMarketFactory.load();
