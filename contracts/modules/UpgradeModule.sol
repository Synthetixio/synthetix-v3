...
contract UpgradeModule is ProxyStorageNamespace, OwnerMixin {
    ...
    function upgradeTo(address newImplementation) public onlyOwnerOrProxy {
        require(newImplementation != address(0), "Invalid impl: zero address");
        require(Address.isContract(newImplementation), "Invalid impl: not a contract");
    ...
    function _requireIsUpgradeable(address newImplementation) private {
        ...
        (bool success, bytes memory data) =
            address(newImplementation).call(abi.encodeWithSelector(UpgradeModule(0).isUpgradeable.selector));

        bool isUpgradeable = abi.decode(data, (bool));

        require(success && isUpgradeable, "Invalid impl: not upgradeable");
    }
    ...
    function canUpgradeAgain() public {
        ...
        (bool success, bytes memory data) =
            address(this).call(abi.encodeWithSelector(KnownImplementation(0).getKnownValue.selector));
