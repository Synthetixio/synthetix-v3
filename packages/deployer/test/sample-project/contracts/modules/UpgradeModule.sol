...
contract UpgradeModule is ProxyStorageNamespace, OwnerMixin {
    ...
    function upgradeTo(address newImplementation) public onlyOwnerOrProxy {
        ...
        require(Address.isContract(newImplementation), "Invalid: not a contract");
    ...
    function _requireIsUpgradeable(address newImplementation) private {
        ...
        (bool success, bytes memory data) =
            address(newImplementation).call(abi.encodeWithSelector(UpgradeModule(address(0)).isUpgradeable.selector));

        bool isUpgradeable = abi.decode(data, (bool));

        require(success && isUpgradeable, "Invalid: not upgradeable");
    }
    ...
    function canUpgradeAgain() public {
        ...
        (bool success, bytes memory data) =
            address(this).call(abi.encodeWithSelector(KnownImplementation(address(0)).getKnownValue.selector));
