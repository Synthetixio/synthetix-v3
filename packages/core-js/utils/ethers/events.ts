 ...
 */
export function findEvent({ receipt, eventName, contract = undefined }: { receipt: ethers.providers.TransactionReceipt, eventName: string, contract?: ethers.Contract }) {
 ...
 * @returns {array} The array of parsed events
 */
export function parseLogs({ contract, logs }: { contract: ethers.Contract, logs: any[] }) { // TODO
  return logs.map((log) => {
