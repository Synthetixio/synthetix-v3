...
describe('VaultRewardsModule', function () {
  ...
  before('deploy fake reward token', async () => {
    ...
    await (
      await Collateral.connect(owner).initialize('Fake Reward', 'FAKE', 18)
    ).wait();
  });
    ...
    describe('instantaneous', () => {
      describe('in the past', () => {
        ...
        it('is distributed', async () => {
          const rewards = await systems().Core.callStatic.getAvailableRewards(fundId, collateralAddress(), accountId);
      ...
      describe('in the future', () => {
        ...
        it('has no rate', async () => {
          const rates = await systems().Core.getCurrentRewardAccumulation(fundId, collateralAddress());
        ...
        describe('after time passes', () => {
          ...
          describe('new distribution', () => {
            ...
            it.skip('distributes portion of rewards immediately', async () => {
              const rewards = await systems().Core.callStatic.getAvailableRewards(fundId, collateralAddress(), accountId);
              // should have received only the one past reward
              assertBn.equal(rewards[0], rewardAmount.add(rewardAmount.mul(75).div(100)).add(rewardAmount.mul(110).div(200)));
            });
