...
import { TestableDistributionModule } from '../../../typechain-types/contracts/modules/test/TestableDistributionModule';
...
describe.only('Distribution', async () => {
  ...
  describe('updateActorValue()', async () => {
    ...
    describe('add actors with value', async () => {
      before(restore);

      before('add shares', async () => {
        await FakeDistributionModule.Distribution_updateActorValue(actor1, bn(50));
        await FakeDistributionModule.Distribution_updateActorValue(actor2, bn(125));
        await FakeDistributionModule.Distribution_updateActorValue(actor3, bn(175));
      });

      it('returns proper values', async () => {
        // value per share is 1 until distributeValue is called
        assertBn.equal(await FakeDistributionModule.Distribution_getActorValue(actor1), bn(50));
        assertBn.equal(await FakeDistributionModule.Distribution_getActorValue(actor2), bn(125));
        assertBn.equal(await FakeDistributionModule.Distribution_getActorValue(actor3), bn(175));
      });

      it('has correct actor shares', async () => {
        // value per share is 1 until distributeValue is called
        assertBn.equal(await FakeDistributionModule.Distribution_getActorShares(actor1), bn(50));
        assertBn.equal(await FakeDistributionModule.Distribution_getActorShares(actor2), bn(125));
        assertBn.equal(await FakeDistributionModule.Distribution_getActorShares(actor3), bn(175));
      });

      it('returns proper total value', async () => {
        assertBn.equal(await FakeDistributionModule.Distribution_totalValue(), bn(350));
      });
    });
  });
  ...
  describe('distributeValue()', async () => {
    ...
    describe('when actors enter using shares and no value', async () => {
      ...
      describe('accumulateActor()', async () => {
        ...
        it('actor1 has correct shares', async () => {
          const currentActorValue = await FakeDistributionModule.Distribution_getActorValue(actor1);
          assertBn.equal(
            await FakeDistributionModule.callStatic.Distribution_accumulateActor(actor1),
            currentActorValue.sub(previousActor1Value)
          );
        });
    ...
    describe('when actors enter with value', async () => {
      ...
      describe('value gets distributed to actors', async () => {
        before('distribute value', async () => {
          await FakeDistributionModule.Distribution_distributeValue(bn(1000));
        });

        it('has correct actor value', async () => {
          const shareValue = wei(1000).div(wei(450)).toBN(); // (1000: distributedValue) / (50 + 150 + 250: totalShares)
          console.log('shareValue: ', shareValue, bn(50).mul(shareValue));
          assertBn.equal(
            await FakeDistributionModule.Distribution_getActorValue(actor1),
            bn(50).mul(shareValue)
          );
          assertBn.equal(
            await FakeDistributionModule.Distribution_getActorValue(actor2),
            bn(150).mul(shareValue)
          );
          assertBn.equal(
            await FakeDistributionModule.Distribution_getActorValue(actor3),
            bn(250).mul(shareValue)
          );
        });
      });
    });
