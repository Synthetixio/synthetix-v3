...
library CurvesLibrary {
    ...
    function calculateValueAtCurvePoint(PolynomialCurve memory curve, uint x) internal pure returns (uint y) {
        ...
        int xInt = int(x);
        return uint(curve.a.decimalMulInt(xInt.decimalMulInt(xInt)) + curve.b.decimalMulInt(xInt) + curve.c);
    }
    ...
    function generateCurve(
        ...
        curve.a = (p1x.decimalMulInt(p3y - p2y) + p2x.decimalMulInt(p1y - p3y) + p3x.decimalMulInt(p2y - p1y)).decimalDivInt(
            ((p1x - p2x).decimalMulInt((p1x - p3x).decimalMulInt(p2x - p3x)))
        );
        curve.b = (p2y - p1y).decimalDivInt(p2x - p1x) - curve.a.decimalMulInt(p1x + p2x);
        curve.c = p1y - curve.a.decimalMulInt(p1x.decimalMulInt(p1x)) - curve.b.decimalMulInt(p1x);

        // TODO REVIEW THIS SANITY CHECK
        int testX = (-curve.b).decimalDivInt((curve.a * 2));
