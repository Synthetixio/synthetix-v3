...
interface IMarketCollateralModule {
     ...
     */
    event MarketCollateralDeposited(uint indexed marketId, address indexed collateralType, uint amount, address indexed sender);

    /**
     * @notice Emitted when `amount` of collateral of type `collateralType` is withdrawn from market `marketId` by `sender`.
     */
    event MarketCollateralWithdrawn(uint indexed marketId, address indexed collateralType, uint amount, address indexed sender);
