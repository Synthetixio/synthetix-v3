...
import { ElementaryTypeName, SourceUnit, TypeName, UserDefinedTypeName } from 'solidity-ast';
...
.setAction(async ({ output, artifact }: { output: string, artifact: string,  }, hre) => {
  ...
  if (!artifactData.isStorageLibrary) {
    throw new Error(`contract ${artifactData.sourceName}:${artifactData.contractName} is not storage library`);
  }
  ...
  const template = path.resolve(__dirname, '..', '..', 'templates', 'TestableStorage.sol.mustache');
  ...
  for (const inputs of _generateTemplateInputs(artifactData.sourceName, artifactData.contractName, astNodes)) {
    const sourceCode = renderTemplate(template, inputs as any);
...
function _generateTemplateInputs(sourceFile: string, contractName: string, sourceUnits: SourceUnit[]) {
  ...
  for (const sourceUnit of sourceUnits) {
    for (const contractDefinition of findAll('ContractDefinition', sourceUnit)) {
      ...
      const dataStructDefinition = Array.from(findAll('StructDefinition', contractDefinition)).find(def => def.name === 'Data');
      ...
      for (const variableDeclaration of dataStructDefinition.members) {
        if (variableDeclaration.typeName?.nodeType === 'Mapping') {
          if (
            variableDeclaration.typeName.valueType.nodeType !== 'ElementaryTypeName'
          ) {
            console.log(`Skipping generated getter/setter for ${variableDeclaration.name} because it has a nested type`);
            continue;
          }
          ...
          if (
            variableDeclaration.typeName.baseType.nodeType !== 'ElementaryTypeName'
          ) {
            console.log(`Skipping generated getter/setter for ${variableDeclaration.name} because it has a nested type`);
            continue;
          }
          ...
          fields.push({
            name: variableDeclaration.name,
            type: (variableDeclaration.typeName! as ElementaryTypeName).name === 'string' || variableDeclaration.typeName.name === 'bytes' ? 
      ...
      for (const functionDefinition of findAll('FunctionDefinition', contractDefinition)) {
        ...
        if (
          storageParams.length > 1
        ) {
          console.log(`Skipping function ${functionDefinition.name} because it contains unsupported storage input parameter type`);
        ...
        methods.push({
          ...
          paramsInject: functionDefinition.parameters.parameters
            .map(p => p.storageLocation === 'storage' ? 'store' : p.name).join(', '),
          returns: functionDefinition.returnParameters.parameters
            .map(p => `${_renderAstType(p.typeName!)}${p.storageLocation !== 'default' ? ' ' + p.storageLocation : ''}`).join(', '),
        })
      }
