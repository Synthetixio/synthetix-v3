...
describe('RewardsManagerModule', function () {
  ...
  describe('No underflow on large delayed token distributions', async () => {
    before(restore);

    before('distribute some reward', async () => {
      const amount = ethers.utils.parseEther('1234567678901234567890');

      await Collateral.mint(RewardDistributor.address, amount);
    ...
    it('Successfully claims with duration delayed', async () => {
      await fastForward(3600 * 14, provider());
  ...
  describe('No underflow on large immediate token distributions', async () => {
    before(restore);
    ...
    it('Successfully claims with duration zero', async () => {
      await systems()
        .Core.connect(user1)
        .claimRewards(accountId, poolId, collateralAddress(), RewardDistributor.address);
    });
